...
export const CurrentSettingsOptions: PomoSettings = {
  ...
  Checkboxes: [
    ...
    },
    {
      ...
      checked: false,
      tooltip:
        "When a pomodoro or break completes, should a notification be sent to the desktop?",
    },
  ],
};

export const LoadSettings = (
  _settings: PomoSettings = CurrentSettingsOptions
): PomoSettings => {
  let newInputSettings = _settings.Inputs.map((item) => GetInputSetting(item));
  let newCheckboxSettings = _settings.Checkboxes.map((item) =>
    GetCheckboxSetting(item)
  );
...
const GetCheckboxSetting = (
  _setting: PomoCheckboxSetting
): PomoCheckboxSetting => {
